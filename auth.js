...
import { GoogleAuthProvider, signInWithPopup, createUserWithEmailAndPassword, signInWithEmailAndPassword, onAuthStateChanged, signOut } from "https://www.gstatic.com/firebasejs/10.13.2/firebase-auth.js"
...
onAuthStateChanged(auth, async (user) => {
  ...
  const protectedPages = ["dashboard.html","budgets.html","reports.html","settings.html","export.html","planner.html","notes.html","help.html","agent.html"]
  const isProtected = protectedPages.includes(file)
  ...
  if (user) {
    ...
    if (isSupport) {
      ...
      if (isIndex) location.replace("dashboard.html")
      if (file === "agent.html") location.replace("help.html")
    }
  } else {
    // After logout, help.html should also redirect to index.html
    if (isProtected) {
